...
import { ArrowRight, Shield, Coins, Building2, Flame } from "lucide-react";
...
function Overview() {
  const { grid, user, hydrated } = useGuardian();
  const stats = cityStats(user.buildings);
  const { current } = levelFor(user.detoxMinutes);

  const preview = grid
    ? calculateReward({ durationMin: 15, intensity: grid.intensity, streakDays: user.streakDays })
    : null;
    ...
    <div className="space-y-6">
      ...
      {!hydrated || !grid ? (
        ...
        <>
          ...
          <section className="glass grid-aura overflow-hidden p-6 sm:p-8 animate-rise">
            <div className="flex flex-wrap items-start justify-between gap-6">
              <div className="max-w-lg">
                <h2 className="flex items-center gap-2 text-xl font-semibold sm:text-2xl">
                  <Shield className="h-5 w-5 text-primary" /> Deploy Detox Shield
                </h2>
                <p className="mt-2 text-sm text-muted-foreground">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2.5">
                  ...
                  <span className="num rounded-full bg-secondary/15 px-3 py-1.5 text-sm font-semibold text-secondary">
                    {preview?.gridMultiplier.toFixed(1) ?? "1.0"}× Grid Bonus
                  </span>
                  {user.streakDays >= 3 && (
              ...
              <Link
                to="/shield"
                className="group inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-4 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-glow)] transition-transform hover:scale-[1.03] active:scale-95"
              >
                ACTIVATE SHIELD
